...
# load COCO class
labels_path = os.path.sep.join([base_path, "coco.names"])
LABELS = open(labels_path).read().strip().split("\n")

# derive paths to YOLO weights and model configuration
weights_path = os.path.sep.join([base_path, "yolov3.weights"])
config_path = os.path.sep.join([base_path, "yolov3.cfg"])
